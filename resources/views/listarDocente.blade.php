                ...
                <table class="table table-striped projects">
                  ...
                  <tbody>
                    @foreach ($instituciones as $institution)
                        <tr>
                          <td>
                            <a>{{$institution->nombre}}</a>
                          </td>
                          <td>
                            <a>{{$institution->codigo}}</a>
                          </td>
                          <td>
                            <!--<a data-toggle="modal" href="#exampleModal" class="btn btn-primary btn-xs"><i class="fa fa-folder"></i> View </a>-->
                            <a href="{{route('editarInstitucion',$institution->id)}}" class="btn btn-info btn-xs">
                              <i class="fa fa-pencil"></i> Editar </a>
                            <!-- Button trigger modal -->
                            <form action="{{route('eliminarInstitucion')}}" method="POST" id="form_x" data-parsley-validate class="form-horizontal form-label-left">
                              {{method_field('DELETE')}}
                               {{ csrf_field() }}

                                  <input type="hidden" readonly id="id" value="{{$institution->id}}" name="id" required="required" class="form-control col-md-7 col-xs-12">
